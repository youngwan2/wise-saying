...
- [방향성] 해당 프로젝트는 현재 구상적인 추가적인 기능과 컨셉에 따라 전면적인 스텍 변경 및 확장이 이루어질 수 있어서 별도의 개발 기간을 명확히 해두지 않았습니다. 전체적인 틀과 초기에 계획했던 부분은 모두 구현이 끝났으나, 사용자 알림 기능, 인공지능 기반 명언 추천 기능 등등 떠오르는 기능의 틀이 명확해 지는대로 대대적인 확장이 있을 예정입니다. 필요에 따라서는 현재 사용되는 스텍을 변경하여 보다 나은 스텍으로 마이그레이션할 수 도 있습니다. 최대한 적용할 수 있는 것들을 적용해보면서 어디까지 발전시켜나갈 수 있는지 도전하기 위한 프로젝트입니다.
...
- 웹 사이트이다 보니 모바일에서 애니메이션이 동작할 때 버벅이는 렉이 발생하는 것을 확인하였습니다(예. 명언 카드의 세부 페이지 이동).(24.04.01).
- 현재 확장가능한 데이터 관리를 위해 데이터베이스 구조 변경 작업 중이며, 보다 유연하고 자연스러운 인터렉션을 위해 UI 애니메이션 추가로 인한 디자인 변경 작업 진행중입니다(24.04.08)
