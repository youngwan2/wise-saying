...
│  │  └─ user
│  ├─ app -----------------→ 페이지
│  │  ├─ (gallery) --------→ 유저의 커스텀 이미지 전시 페이지
│  │  │  └─ gallery
│  │  ├─ (post) -----------→ 유저의 명언 등록 및 수정 페이지
│  │  │  ├─ add-wisesaying
│  │  │  └─ update-wisesaying
│  │  ├─ (quotes) ---------→ 명언/인용문/글귀 등에 관한 페이지
│  │  │  ├─ quotes 
│  │  │  │  ├─ search
│  │  │  │  └─ [category]
│  │  │  │     └─ [name]
...
│  │  │  │     └─ comments
│  │  │  │        └─ route.ts
│  │  │  ├─ revalidate ---→  데이터 재유효화 관련 요청을 처리하는 라우트
│  │  │  └─ users
│  │  │     ├─ mypage
│  │  │     │  ├─ posts
│  │  │     │  └─ upload
│  │  ├─ quotes-styler ---→ 유저의 명언 카드 꾸미기 페이지  
│  │  ├─ layout.tsx
│  │  ├─ loading.tsx
│  │  ├─ not-found.tsx
│  │  ├─ page.tsx
│  ├─ components ---------→ 각 페이지에 쓰이는 컴포넌트 모음
│  │  ├─ Layout
│  │  └─ UI
│  ├─ connect.ts ---------→
│  ├─ custom -------------→ 리액트 커스텀 훅
