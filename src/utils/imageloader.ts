 ...
 */
export const imagePreviewReader = async (e: ChangeEvent<HTMLInputElement>, type: 'mypage' | 'styler') => {
  ...
  if (e.target && e.target.files) {
    try {
      ...
      if (!hasType) return toast.error('jpeg, jpg, png 만 업로드 할 수 있습니다.')

      const src = URL.createObjectURL(file)

      if (type === 'mypage') {
        const isValidFileSize = imageFileSizeChecker(size)
        if (!isValidFileSize) return toast.error('업로드 파일의 사이즈는 500KB를 초과할 수 없습니다.')
        const isValidImageSize = await imageSizeChecker(src)
        if (!isValidImageSize) return toast.error('이미지 크기는 230x230 이하만 가능합니다.')
      }
