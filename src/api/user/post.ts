 ...
 * @param password
 */
export const reqLogin = async (email: FormDataEntryValue | null, password: FormDataEntryValue | null) => {
    ...
    try {
        ...
        const { accessToken, email: dbEmail, status, meg, profile } = await response.json()
...
export const postUserPost = async (
    ...
    if (!(category && wise_sayings && author)) return alert('모든 빈 칸을 채워주세요')
    if (category.toString().length < 1) return alert(`주제를 최소 2자 이상 적어 주세요.`)
    if (wise_sayings.toString().length < 3) return alert(`내용을 최소 3자 이상 적어 주세요.`)
