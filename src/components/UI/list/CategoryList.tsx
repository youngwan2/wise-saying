...
export default function CategoryList({ categoryCount }: { categoryCount: number }) {

  const { items, setSize, size, isLoadingMore, itemCount: currentCount } = useInfiniteScroll('categories', '')
  ...
  return (
    <>
      ...
      <LoadMoreButton onClick={() => setSize(size + 1)} size={size} isLoadingMore={isLoadingMore} maxPage={MAX_PAGE} />
    </>
  )
}
