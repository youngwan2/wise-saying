...
export default function BookmarkList({ bookmarkList, isLoading, isDeleting, onClickDelete }: PropsType) {
        ...
        <div className="px-[1em] mt-[2em] overflow-y-auto overflow-x-hidden min-h-[400px]  p-[1em] w-[90%] mx-auto flex flex-col justify-center items-center">
            {!isLoading ? (
                bookmarkList?.map((bookmark: BookmarkListType) => {
                    return <BookmarkCard key={bookmark.id}  bookmark={bookmark} isDeleting={isDeleting} onClickDelete={onClickDelete} />
                })
            ) : (
