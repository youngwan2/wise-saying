...
export default function CommentList({ id }: PropsType) {
    ...
    const { data: commentsInfo, isLoading } = useSWR<CommentsInfoType>(`/api/quotes/${id}/comments?page=${page}&sort=${sort}`, getCommentsFormDb, {
    ...
    return (
        <section className="py-[1em] ">
            <h2 className="text-white text-[1.5em] mt-[2em]">댓글({!isLoading ? items.length : 0})</h2>
            <CommentForm />
