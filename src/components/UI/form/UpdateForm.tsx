...
export default function UpdateForm() {
    ...
    if (loading) return <h2 className="fixed left-[50%] top-[40%] translate-x-[-50%] translate-y-[-50%] text-[1.5em] p-[15px] rounded-[10px] shadow-[2px_2px_5px_0_rgba(0,0,0,0.5)]">데이터를 불러오는 중입니다...</h2>
    if (error.length > 2) return <h2 className="fixed left-[50%] top-[40%] translate-x-[-50%] translate-y-[-50%] text-[1.5em] p-[15px] rounded-[10px] shadow-[2px_2px_5px_0_rgba(0,0,0,0.5)]">{error}</h2>
    if (!post) return <h2 className="fixed left-[50%] top-[40%] translate-x-[-50%] translate-y-[-50%] text-[1.5em] p-[15px] rounded-[10px] shadow-[2px_2px_5px_0_rgba(0,0,0,0.5)]">포스트가 존재하지 않습니다...</h2>
    return (
        <>
            ...
            <form
                ...
                <article className="px-[2em]">
                    ...
                    }} type="text" className="min-w-[200px] w-[500px] min-h-[40px] px-[10px]" placeholder={"기존내용) " + post.category || "없음"} />
                </article>
                <br />
                {/* 내용 */}
                <article className="px-[2em]">
                    ...
                    }} placeholder={"기존내용) " + post.wise_sayings || "없음"} ></textarea>
                </article>
                <br />
                {/* 작성자(닉네임) */}
                <article className="px-[2em]">
                    ...
                    <input onInput={(e) => {
                        ...
                        setPost({ ...post, author })
                    }} type="text" className="min-w-[200px] w-[500px]  px-[10px] min-h-[40px]" placeholder={"기존내용) " + post.author} />
                </article>
                <br />
                {/* 전송버튼 */}
