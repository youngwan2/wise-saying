...
export default function ImageUploadForm({ setImages, images }: PropsType) {
    ...
    const imageSrcReader = (e: ChangeEvent<HTMLInputElement>) => {
        try {
            if (e.target && e.target.files) {
                const imageSrc = `/images/${e.target.files[0].name}`
    ...
    return (
        <form className="hover:cursor-pointer group min-w-[230px] h-[230px] bg-[white] rounded-[1em]" onSubmit={(e) => {
            e.preventDefault()
        }}>
            <label
            ...
            <input onChange={imageSrcReader} id='image_upload' type="file" className="hidden" />
        </form>
    )
}