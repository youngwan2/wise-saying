...
export default function TextAlignStyler() {
    ...
    return (
        ...
        <>
            <button onClick={onClickSetIsShowOptions} className={`${styles.align_option_button} ${isShowOptions ? 'bg-[#91909049] rounded-[5px]' : ''} hover:text-[#d5d4d4] text-white text-[1.45em] p-[3px] m-[5px]`}>
                {align === 'center' && <BiAlignMiddle />}
                {align === 'left' && <BiAlignLeft />}
                {align === 'right' && <BiAlignRight />}
            </button>
