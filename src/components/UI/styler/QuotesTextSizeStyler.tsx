...
export default function QuotesTextSizeStyler({ sizeUnits, setTextStyleState, textStyle }: PropsType) {
    ...
    return (
        <article>
            ...
            {/* 크기 변경 */}
            <input type="number" className='p-[5px] text-center rounded-[0.5em] shadow-[0_0px_0px_1px_black] w-[200px]' placeholder='Default : 14px' onChange={(e) => {
                const size = Number(e.currentTarget.value)
                setTextStyleState({ ...textStyle, size })
            }} />
            {/* 단위 변경 */}
            <label htmlFor="text-size"></label>
            <select className='bg-[#fae04b] ml-[5px] p-[5px] rounded-[10px]' id='text-size' onChange={(e) => {
