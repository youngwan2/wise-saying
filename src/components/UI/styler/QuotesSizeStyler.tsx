    ...
    return (
            <article className={`${selectTapNum===2? 'block':'hidden'} transition-all`}>
                ...
                <div className="flex items-center text-center">
                    <input aria-label="카드 넓이 사용자 입력창" className="p-[5px] mb-[0.5em] w-[200px] rounded-[10px] " type="number" placeholder="넓이(기본: 350px)" onChange={(e) => {
                        const width = Number(e.currentTarget.value)
                        setSize({ ...size, width })
                    }}></input>
                    <HiX className="text-[1.5em] ml-[5px] pb-[5px]" color="white" />
                    <input aria-label="카드 눂이 사용자 입력창" className="ml-[3px] p-[5px] mb-[0.5em] w-[200px] rounded-[10px]" type="number" placeholder="높이(기본: 400px)" onChange={(e) => {
