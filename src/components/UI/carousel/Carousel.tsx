...
export default function Carousel() {
    ...
    const [images, setImages] = useState(['/images/image1.png', '/images/image2.png', '/images/image3.png', '/images/image4.png', '/images/image5.png', '/images/image6.png', '/images/image7.png'])
    const setImageSrc = useImageElementStore((state) => state.setImageSrc)
    const setImageReset = useImageElementStore((state) => state.setImageReset)
            ...
            <figure className="flex  max-w-[1000px] px-[3em] pb-[5em] mt-[5em]">
                {images.map((image, i) => {
                    return <Image onClick={(e) => {
                        ...
                        setImageSrc(src)
                        setImageReset(false)
                    }} className='rounded-[1em] mx-[10px] shadow-custom1 hover:cursor-pointer' key={image} src={image} alt='명언 카드 배경 이미지' width={230} height={230}></Image>
                })}
            </figure>
            {/* 버튼 컨테이너 */}
