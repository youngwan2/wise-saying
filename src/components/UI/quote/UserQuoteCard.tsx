...
export default function UserQuoteCard({ item, items, index }: PropsType) {
    ...
    const setViews = useCallback(async () => {
        const views = await getQuoteViewsFromDB(quoteId, "user-card-views")
        setViewCount(views)
    }, [quoteId])
    ...
    // 페이지 사전 로드
    function onPrefetch() {
        router.prefetch(`/quotes/authors/${author}/${quoteId}`)
    }
    ...
    const onClickPushAnimation = (e: MouseEvent<HTMLButtonElement>) => {
        viewCounter(quoteId,"user-card-views")
        router.push(`/quotes/authors/${author}/${quoteId}?type=user`)
    }
