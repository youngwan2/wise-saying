...
export default function Page({ params }: { params: { id: string } }) {
    ...
    const getWiseSayingByDay = async (dayOfWeed: string) => {
        ...
        const response = await fetch(`${mode === 'development' ? 'http://localhost:3000/api/items/days/' + dayOfWeed : '/api/days/' + dayOfWeed}`)
    ...
    return (
        <section>
            <h2 className="text-[1.5em] flex items-center p-5"><span className="bg-[#ebbb72] p-[2px] mr-[5px]"><HiCalendar color="white" /></span>{dayOfWeedList[Number(id) - 1]}({items?.length ?? itemCount})</h2>
