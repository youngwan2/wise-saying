...
export async function GET(req: NextRequest) {
    ...
    if (!userId) return NextResponse.json({ meg: '잘못된 요청입니다. 쿼리 변수를 다시 확인해주세요.', status: 403, success: false })
    ...
    return NextResponse.json({ meg: '요청을 완료하였습니다.', status: 200, success: true, items, count:itemCount })
}