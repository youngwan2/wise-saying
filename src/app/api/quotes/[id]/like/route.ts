...
export async function GET(req: NextRequest, res: { params: { id: string } }) {
    ...
    try {
        ...
        return NextResponse.json({ meg: '정상적으로 처리되었습니다.', status: 201, success: true, likeCount, quoteId })

    } catch (error) {
        console.error('GET /api/quotes/[id]/like', error)
        return NextResponse.json({ meg: '서버에서 문제가 발생하였습니다. 나중에 다시시도 해주세요.', status: 500, success: false, likeCount:0 })
    }
}
