    ...
    try {
        switch (type) {
            ...
            `
                ...
                const resultByAuthorCount = await db.get(authorCountQuery, [`%${searchText}%`])
                const resultByKeywordCount = await db.get(keywordCountQuery, [`%${searchText}%`])

                return NextResponse.json({
                    status: 200, meg: '성공적으로 처리 되었습니다.', success: true,
                    totalCounts: {
            ...
            `
                ...
                return NextResponse.json({ status: 200, meg: '성공적으로 처리 되었습니다.', success: true, items: result, totalCount: totalResult.count })
            }
