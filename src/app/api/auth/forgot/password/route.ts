...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (!validEmail) return NextResponse.json({ meg: "유효한 이메일 형식이 아닙니다.", success: false, status: 400 })
        ...
        // 존재하지 않는 이메일
        if (mailCount < 1) return NextResponse.json({ meg: "존재하지 않는 이메일 입니다.", success: false, status: 404 })

        // 도메인 유효성 검증
        const hasDomain = await emailMxValidator(validEmail)
        if (!hasDomain) return NextResponse.json({ meg: "해당 이메일의 도메인이 존재하지 않습니다.", success: false, status: 404 })
        ...
        if (mailCount > 0 && hasDomain) {
            ...
            return NextResponse.json({ meg: "임시 토큰이 발급 처리 되었습니다. 추가 안내를 위해 이메일을 확인해주세요.", success: true, status: 201 })
        }
