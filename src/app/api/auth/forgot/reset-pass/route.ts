...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        // 유저 정보 유효성 검사
        const validUser = await userSchema.validateAsync({ email, password: pass, reConfirmPw: confirm })
        if (!validUser) return NextResponse.json({ meg: "유효한 데이터 형식이 아닙니다. 다시 확인해주세요", status: 400, success: false })
        ...
        return NextResponse.json({ meg: "비밀번호가 재설정 되었습니다.", status: 201, success: true })
