...
export default function ResetPage() {
    ...
    return (
        <form ref={formRef} className="shadow-[inset_-3px_-3px_5px_0_rgba(0,0,0,0.5)] bg-[#d8644f] max-w-[400px] w-full h-[370px] p-[10px] rounded-[10px] absolute left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%]" action={postResetPassAction}>
            <h2 className=" text-[1.5em] text-center my-[0.6em] mb-[0.8em] font-bold text-white">비밀번호 재설정</h2>
            <div className="flex flex-col">
                <label htmlFor="password" className="text-white p-[5px]">새 비밀번호</label>
                <input
            ...
            <br />
            <div className="flex flex-col">
                <label htmlFor="confirm-password" className="text-white  p-[5px]">재확인</label>
                <input
                    className="placeholder:text-[0.85em] text-[1.05em] shadow-[inset_-3px_-3px_5px_0_rgba(0,0,0,0.5)] bg-white  rounded-[20px] p-[8px] px-[10px] focus:outline-none text-[tomato] font-semibold "
            ...
            <button className="shadow-[inset_-3px_-3px_5px_0_rgba(0,0,0,0.5)] hover:text-white hover:bg-[#162557] bg-white w-[130px] relative left-[50%] translate-x-[-50%] mt-[2em] text-[tomato] rounded-[5px] p-[8px] font-bold">전송</button>
        </form>
    )
    ...
    function validPass(pass: string) {
        const regex = /^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^&+=])[a-zA-Z0-9!@#$%^&+=]{8,}$/i
