...
`
export const updateQuoteAction = async (_: { message: string }, form: FormData) => {
    ...
    const postId = form.get('postId')?.valueOf().toString() || ''
    const scrept = process.env.JWT_SCREPT || ''

    // 유효성 검증
    if (!token) return { message: '유효한 접근 권한이 없습니다. 로그인 후 시도해주세요.', success: false }
    ...
    try {
        ...
        await db.query(updateQuery, [content, category, author, postId])
       
        return { message: '성공적으로 수정하였습니다.', success: true }
